...
path_to_preds = ['./results/opportunity/deepconvlstm/inertial/']
seeds = [1]
sampling_rate = 30
majority_filters = [251, 501, 751, 1001, 1251]
json_files = [
    ...
    for f in majority_filters:
        ...
        for s_pos, seed in enumerate(seeds):
            ...
            for i, j in enumerate(json_files):
                ...
                for sbj in val_sbjs:
                    data = pd.read_csv(os.path.join('data/opportunity_adl_data/raw_clipped_scaled/',
                                       sbj + '.csv'), index_col=False).replace({"label": label_dict}).fillna(0).to_numpy()
                    v_data = np.append(v_data, data, axis=0)

                v_preds = np.array([])
                v_orig_preds = np.load(os.path.join(
                    path, 'seed_' + str(seed), 'unprocessed_results/v_preds_opportunity_split_{}.npy'.format(i + 1)))

                for sbj in val_sbjs:
                    sbj_pred = v_orig_preds[v_data[:, 0]
                                            == int(sbj.split("_")[-1])]
                    sbj_pred = [majorityVoting(i, sbj_pred.astype(
                        int), f) for i in range(len(sbj_pred))]
                    v_preds = np.append(v_preds, sbj_pred)

                print("Converting to Segments....")
                seg_data = convert_samples_to_segments(
                    v_data[:, 0], v_preds, sampling_rate)
