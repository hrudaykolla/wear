...
from .libs.utils import (train_one_epoch, ANETdetection,
                         save_checkpoint, make_optimizer, make_scheduler, ModelEma)
...
def run_actionformer(val_sbjs, cfg, ckpt_folder, ckpt_freq, resume, rng_generator, run):
    ...
    print(f"val_dataset.shape {len(val_dataset)}")
    # validation sensor data loading
    val_sens_data = None
    for sbj in val_sbjs:
        sbj_data = pd.read_csv(os.path.join(
            cfg['dataset']['sens_folder'], sbj + '.csv'), index_col=False).replace({"label": cfg['label_dict']}).fillna(0)
    ...
    print("Number of learnable parameters for ActionFormer: {}".format(
    ...
    for epoch in range(start_epoch, start_epoch + max_epochs):
        # train for one epoch
        t_loss = train_one_epoch(train_loader, model, optimizer,
                                 scheduler, model_ema, cfg['train_cfg']['clip_grad_l2norm'])
        ...
        det_eval = ANETdetection(
            val_dataset.json_anno, val_dataset.split[0], tiou_thresholds=val_db_vars['tiou_thresholds'])

        v_loss, v_segments = valid_one_epoch(val_loader, model)
        v_preds, v_gt, _ = convert_segments_to_samples(
            v_segments, val_sens_data, cfg['dataset']['sampling_rate'])
        ...
        conf_mat = confusion_matrix(
            v_gt, v_preds, normalize='true', labels=range(len(cfg['labels'])))
        v_acc = conf_mat.diagonal()/conf_mat.sum(axis=1)
        v_prec = precision_score(
            v_gt, v_preds, average=None, zero_division=1, labels=range(len(cfg['labels'])))
        v_rec = recall_score(v_gt, v_preds, average=None,
                             zero_division=1, labels=range(len(cfg['labels'])))
        ...
        block4 = ''
        block4 += '\t\tAvg. mAP {:>4.2f} (%) '.format(
            np.nanmean(val_mAP) * 100)
        ...
        if run is not None:
            run[split_name].append({"train_loss": t_loss, "val_loss": v_loss, "accuracy": np.nanmean(v_acc), "precision": np.nanmean(
                v_prec), "recall": np.nanmean(v_rec), 'f1': np.mean(v_f1), 'mAP': np.mean(val_mAP)}, step=epoch)
            for tiou, tiou_mAP in zip(cfg['dataset']['tiou_thresholds'], val_mAP):
