...
from inertial_baseline.DeepConvLSTM1 import DeepConvLSTM1
from inertial_baseline.DeepConvLSTMwithAttention import DeepConvLSTMwithAttention
...
def run_inertial_network(train_sbjs, val_sbjs, cfg, ckpt_folder, ckpt_freq, resume, rng_generator, run):
    ...
    train_data, val_data = np.empty(
        (0, cfg['dataset']['input_dim'] + 2)), np.empty((0, cfg['dataset']['input_dim'] + 2))
    for t_sbj in train_sbjs:
        t_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], t_sbj + '.csv'),
    ...
    for v_sbj in val_sbjs:
        v_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], v_sbj + '.csv'),
                             index_col=False).replace({"label": cfg['label_dict']}).fillna(0).to_numpy()
        val_data = np.append(val_data, v_data, axis=0)

    # define inertial datasets
    train_dataset = InertialDataset(
        train_data, cfg['dataset']['window_size'], cfg['dataset']['window_overlap'])
    test_dataset = InertialDataset(
        val_data, cfg['dataset']['window_size'], cfg['dataset']['window_overlap'])

    # define dataloaders
    train_loader = DataLoader(train_dataset, cfg['loader']['batch_size'], shuffle=True, num_workers=4,
                              worker_init_fn=worker_init_reset_seed, generator=rng_generator, persistent_workers=True)
    test_loader = DataLoader(test_dataset, cfg['loader']['batch_size'], shuffle=False, num_workers=4,
                             worker_init_fn=worker_init_reset_seed, generator=rng_generator, persistent_workers=True)
    # define network
    if cfg['name'] == 'deepconvlstm':
        ...
        print("Number of learnable parameters for DeepConvLSTM: {}".format(
            sum(p.numel() for p in net.parameters() if p.requires_grad)))
    elif cfg['name'] == 'deepconvlstm1':
        net = DeepConvLSTM1(
            train_dataset.channels, train_dataset.classes, train_dataset.window_size,
            cfg['model']['conv_kernels'], cfg['model']['conv_kernel_size'],
            cfg['model']['lstm_units'], cfg['model']['lstm_layers'], cfg['model']['dropout']
        ...
        print("Number of learnable parameters for DeepConvLSTM: {}".format(
            sum(p.numel() for p in net.parameters() if p.requires_grad)))
    elif cfg['name'] == 'deepconvlstmwithattention':
        net = DeepConvLSTMwithAttention(
            train_dataset.channels, train_dataset.classes, train_dataset.window_size,
            cfg['model']['conv_kernels'], cfg['model']['conv_kernel_size'],
            cfg['model']['lstm_units'], cfg['model']['lstm_layers'], cfg['model']['dropout']
        )
        ...
        net = AttendAndDiscriminate(
            train_dataset.channels, train_dataset.classes, cfg['model']['hidden_dim'], cfg['model']['conv_kernels'], cfg['model']['conv_kernel_size'], cfg['model']['enc_layers'], cfg[
                'model']['enc_is_bidirectional'], cfg['model']['dropout'], cfg['model']['dropout_rnn'], cfg['model']['dropout_cls'], cfg['model']['activation'], cfg['model']['sa_div']
        )
        print("Number of learnable parameters for A-and-D: {}".format(sum(p.numel()
              for p in net.parameters() if p.requires_grad)))

    # define criterion and optimizer
    criterion = nn.CrossEntropyLoss()
    opt = torch.optim.Adam(net.parameters(
    ), lr=cfg['train_cfg']['lr'], weight_decay=cfg['train_cfg']['weight_decay'])
    ...
    if cfg['train_cfg']['lr_step'] > 0:
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, cfg['train_cfg']['epochs'], eta_min=0, last_epoch=- 1, verbose=True)

    # use weighted loss if selected
    if cfg['train_cfg']['weighted_loss']:
        class_weights = compute_class_weight('balanced', classes=np.unique(
            train_dataset.labels) + 1, y=train_dataset.labels + 1)
        criterion.weight = torch.tensor(
            class_weights).float().to(cfg['devices'][0])

    if resume:
        if os.path.isfile(resume):
            checkpoint = torch.load(
                resume, map_location=lambda storage, loc: storage.cuda(cfg['devices'][0]))
    ...
    for epoch in range(start_epoch, cfg['train_cfg']['epochs']):
        # training
        net, t_losses, _, _ = train_one_epoch(
            train_loader, net, opt, criterion, cfg['devices'][0])
        ...
        if (((epoch + 1) == cfg['train_cfg']['epochs']) or ((ckpt_freq > 0) and ((epoch + 1) % ckpt_freq == 0))):
            ...
            save_checkpoint(save_states, False, file_folder=os.path.join(
                ckpt_folder, 'ckpts'), file_name=file_name)

        # validation
        v_losses, v_preds, v_gt = validate_one_epoch(
            test_loader, net, criterion, cfg['devices'][0])
        ...
        det_eval = ANETdetection(
            cfg['dataset']['json_anno'], 'validation', tiou_thresholds=cfg['dataset']['tiou_thresholds'])
        # undwindow inertial data (sample-wise structure instead of windowed)
        v_preds, v_gt = unwindow_inertial_data(
            val_data, test_dataset.ids, v_preds, cfg['dataset']['window_size'], cfg['dataset']['window_overlap'])
        # convert to samples (for mAP calculation)
        v_segments = convert_samples_to_segments(
            val_data[:, 0], v_preds, cfg['dataset']['sampling_rate'])
        ...
        if epoch == (start_epoch + cfg['train_cfg']['epochs']) - 1:
            ...
            np.save(os.path.join(ckpt_folder, 'unprocessed_results',
                    'v_preds_' + split_name), v_preds)
            np.save(os.path.join(ckpt_folder, 'unprocessed_results',
                    'v_gt_' + split_name), v_gt)
            v_results.to_csv(os.path.join(
                ckpt_folder, 'unprocessed_results', 'v_seg_' + split_name + '.csv'), index=False)
        ...
        block1 = 'Epoch: [{:03d}/{:03d}]'.format(
            epoch, cfg['train_cfg']['epochs'])
        ...
        if run is not None:
            run[split_name].append({"train_loss": np.nanmean(t_losses), "val_loss": np.nanmean(v_losses), "accuracy": np.nanmean(
                v_acc), "precision": np.nanmean(v_prec), "recall": np.nanmean(v_rec), 'f1': np.nanmean(v_f1), 'mAP': np.nanmean(v_mAP)}, step=epoch)
            for tiou, tiou_mAP in zip(cfg['dataset']['tiou_thresholds'], v_mAP):
                run[split_name].append(
                    {'mAP@' + str(tiou): tiou_mAP}, step=epoch)
...
def run_inertial_network_for_features(net, train_sbjs, val_sbjs, itr, cfg, gpu=None):
    for t_sbj in train_sbjs:
        ...
        # define inertial datasets
        train_dataset = InertialDataset(
            t_data, cfg['dataset']['window_size'], cfg['dataset']['window_overlap'])
        train_loader = DataLoader(train_dataset, len(train_dataset), shuffle=False,
                                  num_workers=4, worker_init_fn=worker_init_reset_seed, persistent_workers=True)
        net.to(cfg['devices'][0])
        net.eval()
        with torch.no_grad():
            # iterate over validation dataset
        ...
        if net.feature_extract == 'conv':
            destination_folder = os.path.join(
                cfg['dataset']['conv_feat_folder'], cfg['dataset_name'] + '_split_' + str(itr+1))
        elif net.feature_extract == 'lstm':
            destination_folder = os.path.join(
                cfg['dataset']['lstm_feat_folder'], cfg['dataset_name'] + '_split_' + str(itr+1))
