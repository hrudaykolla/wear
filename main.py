# ------------------------------------------------------------------------
# Main script to commence baseline experiments on any HAR dataset
# ------------------------------------------------------------------------
# Author: Marius Bock
...
def main(args):
    ...
    rng_generator = fix_random_seed(
        config['init_rand_seed'], include_cuda=True)

    all_v_pred = np.array([])
    all_v_gt = np.array([])
    all_v_mAP = np.empty((0, len(config['dataset']['tiou_thresholds'])))
    if config['name'] == 'tridet' or config['name'] == 'actionformer':
    ...
    for i, anno_split in enumerate(config['anno_json']):
        ...
        config['label_dict'] = dict(
            zip(config['labels'], list(range(len(config['labels'])))))
        train_sbjs = [x for x in anno_file if anno_file[x]
                      ['subset'] == 'Training']
        ...
        if config['name'] == 'deepconvlstm' or config['name'] == 'attendanddiscriminate' or config['name'] == 'deepconvlstm1' or config['name'] == 'deepconvlstmwithattention':
            t_losses, v_losses, v_mAP, v_preds, v_gt, net = run_inertial_network(
                train_sbjs, val_sbjs, config, log_dir, args.ckpt_freq, args.resume, rng_generator, run)
            ...
            if config['save_conv_features_during_training'] == True:
                ...
                run_inertial_network_for_features(
                    net, train_sbjs, val_sbjs, i, config, config['devices'][0])
            ...
            if config['trained_on_inertial_extracted_features'] == True:
                config['dataset']['feat_folder'] = config['main_feat_folder'] + \
                    config['dataset_name'] + '_split_' + str(i+1) + '/'
                print(config['dataset']['feat_folder'])
            t_losses, v_losses, v_mAP, v_preds, v_gt = run_actionformer(
                val_sbjs, config, log_dir, args.ckpt_freq, args.resume, rng_generator, run)
        elif config['name'] == 'tridet':
            if config['trained_on_inertial_extracted_features'] == True:
                config['dataset']['feat_folder'] = config['main_feat_folder'] + \
                    config['dataset_name'] + '_split_' + str(i+1) + '/'
                print(config['dataset']['feat_folder'])
            t_losses, v_losses, v_mAP, v_preds, v_gt = run_tridet(
                val_sbjs, config, log_dir, args.ckpt_freq, args.resume, rng_generator, run)

        # raw results
        conf_mat = confusion_matrix(
            v_gt, v_preds, normalize='true', labels=range(len(config['labels'])))
        v_acc = conf_mat.diagonal()/conf_mat.sum(axis=1)
        v_prec = precision_score(
            v_gt, v_preds, average=None, zero_division=1, labels=range(len(config['labels'])))
        v_rec = recall_score(v_gt, v_preds, average=None,
                             zero_division=1, labels=range(len(config['labels'])))
        v_f1 = f1_score(v_gt, v_preds, average=None,
                        zero_division=1, labels=range(len(config['labels'])))
        ...
        conf_disp = ConfusionMatrixDisplay(
            confusion_matrix=conf_mat, display_labels=config['labels'])
    ...
    conf_mat = confusion_matrix(
        all_v_gt, all_v_pred, normalize='true', labels=range(len(config['labels'])))
    v_acc = conf_mat.diagonal()/conf_mat.sum(axis=1)
    v_prec = precision_score(all_v_gt, all_v_pred, average=None,
                             zero_division=1, labels=range(len(config['labels'])))
    v_rec = recall_score(all_v_gt, all_v_pred, average=None,
                         zero_division=1, labels=range(len(config['labels'])))
    v_f1 = f1_score(all_v_gt, all_v_pred, average=None,
                    zero_division=1, labels=range(len(config['labels'])))
    ...
    plt.close()
    if run is not None:
        run['conf_matrices'].append(
            File(os.path.join(log_dir, 'all_raw.png')), name='all')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--config', default='./configs/60_frames_30_stride/tridet_combined.yaml')
