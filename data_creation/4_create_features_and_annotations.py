...
inertial_folder = './data/wear/processed/inertial_features/{}_frames_{}_stride'.format(
    frames, stride)
i3d_folder = './data/wear/processed/i3d_features/{}_frames_{}_stride'.format(
    frames, stride)
combined_folder = './data/wear/processed/combined_features/{}_frames_{}_stride'.format(
    frames, stride)
    ...
    for sbj in split_sbjs:
        ...
        try:
            output_combined = np.concatenate(
                (output_inertial, output_i3d), axis=1)
        except ValueError:
            print('had to chop')
            output_combined = np.concatenate(
                (output_inertial[:output_i3d.shape[0], :], output_i3d), axis=1)
